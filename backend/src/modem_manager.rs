...
\tRX bytes OK: 1092";
    ...
    #[test]
    fn dedups_stale_data_bearers_after_numeric_path_sort() {
        let mut paths = vec![
            "/mm/Bearer/10".to_string(),
            "/mm/Bearer/2".to_string(),
            "/mm/Bearer/9".to_string(),
        ];
        sort_bearer_paths(&mut paths);
        assert_eq!(
            paths,
            vec![
                "/mm/Bearer/2".to_string(),
        ...
        let contexts = paths
            .iter()
            .map(|path| apn_ctx(path, "internet", "more", "dual", false))
            .collect();

        let deduped = dedup_apn_contexts(contexts);

        assert_eq!(deduped.len(), 1);
        assert_eq!(deduped[0].path, "/mm/Bearer/10");
    }
...
fn bearer_path_numeric_suffix(path: &str) -> Option<(&str, u64)> {
    let (prefix, suffix) = path.rsplit_once('/')?;
    let number = suffix.parse::<u64>().ok()?;
    Some((prefix, number))
}

fn compare_bearer_paths(a: &str, b: &str) -> std::cmp::Ordering {
    match (bearer_path_numeric_suffix(a), bearer_path_numeric_suffix(b)) {
        (Some((a_prefix, a_number)), Some((b_prefix, b_number))) if a_prefix == b_prefix => {
            a_number.cmp(&b_number).then_with(|| a.cmp(b))
        }
        _ => a.cmp(b),
    }
}

fn sort_bearer_paths(paths: &mut Vec<String>) {
    paths.sort_by(|a, b| compare_bearer_paths(a, b));
    paths.dedup();
}
